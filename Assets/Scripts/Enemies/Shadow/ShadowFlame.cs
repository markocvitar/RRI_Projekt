...
public class ShadowFlame : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        shadowflameDamage = (int)Enemy.GetComponent<EnemyDamage>().damage;
    ...
    private void OnTriggerEnter2D(Collider2D other) {
        if(other.CompareTag("Player")){
            other.gameObject.GetComponent<PlayerHealth>().TakeDamage(shadowflameDamage, gameObject);
            Destroy(gameObject);
